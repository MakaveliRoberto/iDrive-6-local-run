...
#
# 2011/05: New feature: Copy /var/dump to an attached USB stick (recursively). A directory "HBCoreUpload" has to be existent in root folder of USB device.
#
#set -x
...
if [[ $? -eq 0 ]]
then
   echo "$0: Found a USB device" > $SLOG

   for MY_LINE in $($LS_CMD)
   do
      echo "$0: Checking $MY_LINE" > $SLOG
...
while [[ $(pidin -p HBFileUpload -fa | grep -E [0-9]+) == "" || $HBFILEUPLOAD_STARTED -eq 0 ]]; do
   ...
   /opt/sys/bin/HBFileUpload -v -D ${COREDIR_PREFIX} -r -i 5000 -d 20 > /dev/console 2>&1 &
   HBFILEUPLOADPID=$!
   echo "$0: HBFileUpload has PID $HBFILEUPLOADPID" > $SLOG
   sleep $INITIALWAIT
   ls /proc/$HBFILEUPLOADPID > /dev/null 2>&1
   if [[ $? -eq 0 ]]; then
      HBFILEUPLOAD_STARTED=1
